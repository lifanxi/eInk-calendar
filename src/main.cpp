...
const uint16_t SMARTCONFIG_QR_CODE_WIDTH = 228; // 替换了原来的图片，分辨率也改变
const uint16_t SMARTCONFIG_QR_CODE_HEIGHT = 269;
GeoInfo gi;
...
void ShowWiFiSmartConfig()
{
  ...
  do
  {
    // 替换原先的配置方式，不再使用微信二维码配置，改用ESPTouch组播的形式
    DrawMultiLineString("请使用 ESPTouch 配置无线网络，ESPTouch 可在应用商店下载，配置选项如上。", 90, tipsY, 300, 30);
  } while (display.nextPage());
  drawBitmapFromSpiffs_Buffered("SetStep.bmp", x, y-50, false, true, false);
}
...
void ShowWeatherFoot()
{
  ...
  foot.concat("°C");
   //Serial.printf("llll: %d\n", caq.category.length());

  u8g2Fonts.setFont(u8g2_mfyuehei_14_gb2312);
  if (caq.category.length() >= 12){
    u8g2Fonts.drawUTF8(70, DISPLAY_HEIGHT - 24, foot.c_str());
  }
  else{
